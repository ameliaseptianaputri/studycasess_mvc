...
class BukuModel {
    ...
    public function cari()
    {
        ...
        $this->db->query("SELECT * FROM " . $this->table . " WHERE nama_peminjaman LIKE :nama_peminjaman"); //untuk nyiapin sebuah query sql, terus menggabungkan teks "SELECT * FROM " dengan nama tabel yang disimpan dalam properti `$this->table`
        //Tanda persentase (`%`) digunakan untuk mencari data yang mirip dalam kolom `nama_peminjaman`.
        $this->db->bind('nama_peminjaman', '%' . $buku . '%');//untuk mengikat nilai yang di ambil dari input pencarian $buku kedalam parameter `:nama_peminjaman`
        //`resultSet()`  untuk mengambil hasil query dari database.
        return $this->db->resultSet();//menjalanlan query yang telah di siapkan, kemudian mengembalikan hasil pencarian sebagai kumpulan data
    }
